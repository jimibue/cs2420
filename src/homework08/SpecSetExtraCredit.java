 ...
 */
public class SpecSetExtraCredit<E extends Comparable<E>>
{
     ...
     */
    public SpecSetExtraCredit(int avlDif)
    {
    	this.avlDif= avlDif; 
    	size=0;
    	root = null;
		...
		while(true)
		{
			...
			else if(value < 0)//search right
			{
				if(tempNode.right ==null)//found position
				{
					...
					recursiveBalance(tempNode,this.avlDif);
					return;
				}
...
//		   } 
		...
		//change the reference to the value
		 ...
		 recursiveBalance(focusNode.parent,this.avlDif);
		size--;
		//balance
    ...
    public int height(Node cur) {
    	 ...
    	 public void recursiveBalance(Node cur, int avlDif) {
    		  ...
    		  if(cur.parent!=null) {
    		   recursiveBalance(cur.parent,this.avlDif);
    		  } 
    		  else {
    		   this.root = cur;
