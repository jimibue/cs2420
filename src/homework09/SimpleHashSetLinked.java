 ...
 * @version Spring 2014
 */
public class SimpleHashSetLinked
{
    // Instance variables.
    ...
    protected ArrayList<Object> []      llSize;
     ...
     */
    public SimpleHashSetLinked (int capacity, int elementCount)
    {
    	this.size = 0;
    	totalVisit =0;
        ...
        this.totalCount = 0;
        arrayOfCounters = new SpecificProbeCounter[elementCount+1];
        llSize = new ArrayList[capacity+1];
        
        for(int i = 0; i< elementCount+1; i++){
       	SpecificProbeCounter temp = new SpecificProbeCounter(i);
//        	llSize[i] = new ArrayList<Object>();
...
//            {
            	...
            	//if it is not in the set add else don't
            	if(!(llSize[hash].contains(element)))
            	{
            		size++;
            		llSize[hash].add(element);
            		llSize[hash].indexOf(element);
            		arrayOfCounters[size].thisProbeCount+=llSize[hash].indexOf(element)+1;
            		arrayOfCounters[size].timesVisted++;
            		return;
            	}
            	else
            	{
            		arrayOfCounters[size].thisProbeCount+=llSize[hash].size();
            		arrayOfCounters[size].timesVisted++;
            	}
...
//            	return;
    ...
    {
    	int hash = Math.abs(element.hashCode())%capacity;
    	//arrayOfCounters[hash].timesVisted++;
