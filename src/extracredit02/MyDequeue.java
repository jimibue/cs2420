...
public class MyDequeue<T extends Comparable<T>> 
{
	...
	public T removeMax()
	{
		...
		Item tempItem1 = minHeap.get(tempItem.maxIndex);
		tempItem.maxIndex=0;
		maxHeapSize--;
		...
		minHeapSize--;
		minHeap.add(tempItem.minIndex, replacement );
		replacement.minIndex = tempItem1.minIndex;
		replacement.maxIndex = tempItem.maxIndex+1;
		
		//check parent
		int replacementParent =(replacement.minIndex-1)/2;
		if(specialCompareTo(replacement.getData(), minHeap.get(replacementParent).getData()) <0 )
			perculateUpMinHeap(replacement.minIndex);
		
		else
			percaulteDownMin(replacement.minIndex);
	...
	public T removeMin()
	{
		...
		//hard part////
		Item tempItem1 = minHeap.remove(tempItem.maxIndex);
		Item replacement = minHeap.remove(minHeap.size()-1);
		maxHeap.add(tempItem.minIndex, replacement );
